...
4. Перегенерируйте manifest, чтобы меню увидело новые данные (включая предметы, сводки и ссылки):
   ...
   npm run generate:manifest
   ```
   Скрипт обходит каталоги предметов, собирает метаданные и формирует `public/assets/quizzes/quizzes.json` (манфест v2 с разделами и вложенными викторинами).
...
- Используйте `?subject=<subjectId>`, чтобы открыть меню сразу с нужным разделом. Пример: `index.html?subject=art-of-speech`.
- В этом режиме карточка предмета раскрывается автоматически. Вверху появится ссылка «← Все предметы», чтобы вернуться к общему меню. Ссылки на конкретные викторины (`?quiz=<id>`) продолжают работать независимо от активного предмета.
